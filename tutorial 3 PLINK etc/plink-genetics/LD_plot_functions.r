...
plotPairwiseLD(ALL_maf_R2, title = "ALL Pairwise")
plotPairwiseLD(afr_maf_R2, title = "AFR Pairwise")
plotPairwiseLD(han_maf_R2, title = "HAN Pairwise")
#on x axis, position in chromosome (KBp). Han chinese has correlations between vary different SNPs. 
#"Triangles" mean that SNPs within them are correlated. => plot shows how related pop is => more "triangles", more related
...
plotDecayLD(ALL_maf_R2, title = "ALL Decay")
plotDecayLD(afr_maf_R2, title = "AFR Decay")
plotDecayLD(han_maf_R2, title = "HAN Decay")
#Line not perfect, too little data. In the beginning (on the left), when SNPs are very close (location), they are correlated a lot. 
#With increasing distance, this correlation decreases. (Han shows best example)
#Steeper decay shows that SNPs are less correlated overall. => shows diversity in sample (steeper = more diverse)
